            ...
            let variants = fs::read_dir(&path)
                ...
                .filter_map(|v| {
                    v.ok().and_then(|v_entry| {
                        v_entry.path().file_name().and_then(|n| n.to_str()).map(|s| s.to_string())
                    })
                })
                .collect::<Vec<String>>();
...
pub fn read_body_diffuse_from_variant(decal_path: &std::path::Path, variant_name: &str) -> Result<String, String> {
...
pub fn remove_decal_variant_logic(decal_name: &str, variant_name: &str) -> Result<(), String> {
    ...
    if fs::read_dir(decal_path.clone()).map_err(|e| format!("Failed to read decal directory: {}", e))?.next().is_none() {
